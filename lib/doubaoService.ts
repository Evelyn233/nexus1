  ...
  try {
    ...
    return {
      ...
      if (fetchError.name === 'AbortError') {
        console.error('⏱️ [DOUBAO] API request timeout (30s), switching to fallback')
        return await fallbackToLocalAPI(messages)
      }
      throw fetchError
    }
...
请直接返回3个问题，用换行分隔，不要其他格式。`
    ...
    if (data.choices && data.choices[0] && data.choices[0].message) {
      ...
      clearTimeout(timeoutId)
      if (fetchError.name === 'AbortError') {
        console.error('⏱️ [DOUBAO-SERVICE] API request timeout (30s)')
        // 返回默认问题
        return [
          `关于您提到的"${userInput}"，您希望如何处理这个情况？`,
          '您目前最关心的是什么问题？',
...
export async function generateDeepQuestions(
  ...
  // 🚨 强制3轮对话上限检查（最高优先级）
  console.log(`🔍 [DEEP-QUESTIONS] 3轮检查 - previousAnswers数量: ${previousAnswers?.length || 0}`)
  console.log(`🔍 [DEEP-QUESTIONS] 3轮检查 - previousAnswers内容:`, previousAnswers)
  
  if (previousAnswers && previousAnswers.length >= 3) {
    console.log('🚨🚨🚨 [DEEP-QUESTIONS] 已达到3轮对话上限，强制停止提问！')
    return {
      success: true,
      questions: [],
      shouldStop: true,
      reason: '已完成3轮对话，信息收集完毕'
    }
  }
  
  console.log('✅ [DEEP-QUESTIONS] 未达到3轮上限，继续生成问题')
...
**🚨🚨🚨 最最最高优先级：3轮对话上限（超过 = 立即死刑！）🚨🚨🚨**

当前已有回答数: ${previousAnswers?.length || 0}

**如果已有回答数 >= 3，立即返回空数组 {"questions": []}，绝对不能再问任何问题！**
...
- 用户用中文输入 → 问题用中文
- 用户用英文输入 → 问题用英文
- ❌ 死刑案例：用户中文输入，你生成英文问题 → 死刑！

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**🚨🚨🚨 第二优先级：基于用户最新输入生成问题（违反 = 死刑！）🚨🚨🚨**

**核心原则：**
- ✅ 问题必须100%基于用户最新输入"${userInput}"生成
- ✅ **直接问用户更多信息**：如生活、工作、爱好、经历、想法、目标等，可追问用户刚说的内容，也可问其他维度
- ❌ 绝对不能引用用户没有提到的词汇（如用户没说过"没意思"，就不能问关于"没意思"的问题）
- ❌ 绝对不能从之前的对话历史中"记住"词汇并错误地使用
- ✅ 只能问用户最新输入中明确提到的内容
- 🚩【关键约束】每一轮问题都必须紧扣用户"最新这一句话"的具体内容展开，根据用户最新回答动态生成问题，不要机械套用模板

**示例：**
- 用户最新输入："就是他想的都是很美好的愿景 然后不落地 就是就活在幻想里..."
- ✅ 正确：基于这句问更多信息，如"当他提出那些美好愿景时，你心里在想什么？"、"他说这些话时，你是怎么回应他的？"
- ❌ 错误："当时你听到他说'没意思'时,心里是什么感觉？" ← 用户根本没说过"没意思"！
...
🚨🚨🚨 特别警告：相似问题检测（死刑！）
- ❌ 如果之前问过"画面/形象/镜头"，不能再问"画一幅画/画像/拍成镜头"等相似问题！
- ❌ 示例：第1问问了"你心里的形象有哪些画面？拍成镜头是怎样？"，第2问绝对不能再问"如果画一幅画像会是什么样？"（这是死刑错误！）
- ✅ 正确做法：第2问必须基于用户最新回答的具体内容展开新问题
...
**🚨🚨🚨 重要：问题必须基于用户最新输入"${userInput}"生成！不能引用用户没有提到的词汇！** 🚨🚨🚨

**第一步：识别用户输入的核心是什么**
从用户最新输入"${userInput}"中提取：
...
${previousQuestions && previousQuestions.length > 0 ? (() => {
  const pq = previousQuestions.join(' ').toLowerCase();
  const isOriginStoryLike = (
    /unique story|独特的故事|story about your journey|journey that comes|defines you|来自.*故事|story you have/.test(pq) ||
    (pq.includes('story') && (pq.includes('journey') || pq.includes('unique'))) ||
    /inspired|inspiration|story behind|what'?s? the story|what led you|what made you|why become|why did you become|decision.*story|背后的故事|是什么激发|为什么成为|什么让你|what.*story|story.*decision/.test(pq)
  );
  const isHobbiesLike = /enjoy.*outside|outside.*work|hobbies|life outside|工作外|爱好|喜欢做什么|what do you enjoy|any hobbies|things you love|love doing/.test(pq);
  if (!isOriginStoryLike && !isHobbiesLike) return '';
  const qList = previousQuestions!.map((q, i) => `${i + 1}. ${q}`).join('\n');
  const extra = isHobbiesLike ? `
- **已问过生活/爱好/工作外喜欢什么**。**禁止**同维度换说法再问（如不再问 "Any hobbies?" / "What do you enjoy?"）。但**可以追问**：针对用户**刚回答的具体内容**深入问（如用户说喜欢跑步 → 问「为什么喜欢/什么时候开始的」），**然后**再问其他维度。` : '';
  return `
**🚨 追问多样性（profile 模式）：可以追问，其他维度必选！**
**已问过的问题：**
${qList}

- **每次都是：先追问 → 再问其他的。** 用户输入 XX → **先追问 XX**（针对用户说的深入问）→ **再问其他的**（其他维度）。不要跳过追问。
- 以上已涉及「founder/职业」或「生活/爱好」类。**禁止**同维度换说法再问。**针对用户回答追问**（追问 XX）**要先做**；**然后**问其他维度（若问过 founder 类→问生活/爱好、谁影响你等；若问过爱好→问 founder 相关、挑战、谁影响你等）。
${extra}
- **只输出一个问句**。可以追问，每次**一个问句**。
- 示例：用户说喜欢跑步 → 追问「为什么喜欢跑步？」；**然后**下一问可问其他维度如「谁对你影响比较大？」。
`;
})() : ''}
...
` : '**这是第一个问题**。⚠️ 欢迎语已说过「我们想贴你的故事」，不要重复。\n\n**第一步：判断用户元键入"' + (initialPrompt || userInput) + '"是什么类型**（身份/职业、生活/爱好、情绪、具体事件、身份幻想 等）。\n\n**第二步：第一问问「其他维度」** — 问与用户刚提到的类型**不同**的维度：\n- 若类型=身份/职业 → 问生活/爱好、谁影响你、典型的一天等\n- 若类型=生活/爱好 → 问职业/工作、谁影响你等\n- 若类型=情绪 → 问发生了什么、具体场景\n- 若类型=具体事件 → 问感受、和谁、前后做了什么\n- 若类型=身份幻想 → 先问"为什么"，再问其他维度\n\n**只输出一个问句**，不要两句同义。只输出问题，不要加 "We\'d love to share your story..."。'}
...
**🔥 职业/工作 → 追问生活偏好（重要！）：**
- 若用户元键入或回答中提到**职业、工作**（如 founder、产品、创业、AI、influencer、job、work、profession、产品经理、创业者等）→ 下一问要追问**生活喜欢什么** / **生活中有什么爱好**。
- 中文示例："你生活中喜欢做什么？有什么爱好？"
- 英文示例："What do you enjoy in life? Any hobbies or things you love doing outside work?"
...
**🔥🔥🔥 语言检测（死刑线！）：**
- 用户输入语言：${isChinese ? '中文' : '英文'}
- **问题必须用${isChinese ? '中文' : '英文'}！绝对不能用${isChinese ? '英文' : '中文'}！**
...
**用户最新输入（当前这轮对话的核心内容，必须据此生成问题）：**
"${userInput}"

**用户元键入（初始输入，用于上下文理解）：**
"${initialPrompt || userInput}"

⚠️⚠️⚠️ **强制要求：问题必须基于"用户最新输入"生成！不能引用之前对话中没有的内容！** ⚠️⚠️⚠️
...
🚨 这是第一个问题！
**要求：** 欢迎语已说明「我们想贴你的故事」，不要重复。**先判断用户输入是什么类型**（身份/职业、生活/爱好、情绪、具体事件、身份幻想），**再问「其他维度」**（与该类型不同的维度）。只输出问题。
**类型 → 其他维度：** 身份/职业→问生活/爱好/谁影响你；生活/爱好→问职业/谁影响你；情绪→问发生了什么/场景；具体事件→问感受/和谁；身份幻想→先问为什么再问其他。
**第2问及以后：** 先追问用户刚说的 → 再问其他维度。禁止重复已问、禁止重复已答。**只输出一个问句**。
**若已问过某维度** → **先**追问用户刚说的，**然后**问其他维度（其他维度必选）。
`}
...
**检查清单：**
1. ✅ 问题是否基于用户最新输入"${userInput}"？
2. ✅ 问题中是否引用了用户没有提到的词汇？（如用户没说过"没意思"，就不能问"没意思"）
3. ✅ 问题是否问的是用户已经回答过的内容？
4. ✅ 问题是否包含用户输入中没有的情绪词或事件？

**⚠️ 禁止事项：**
- ❌ 禁止引用用户没有提到的词汇（如用户没说过"没意思"，就不能问关于"没意思"的问题）
- ❌ 禁止问之前对话中已经回答过的问题
- ❌ 禁止假设用户没有表达的情绪（如用户没说"生气"，就不要问"为什么生气"）

**正确做法：**
- ✅ 只基于用户最新输入"${userInput}"中的具体内容提问
- ✅ 如果用户提到"愿景"、"不落地"、"幻想"、"工程问题"，可以问这些内容
- ✅ 如果用户提到"他说"，可以问"他说了什么"或"他说的哪句话让你有这种感觉"
- ❌ 绝对不能问用户没有提到的具体词汇（如"没意思"）
...
现在生成问题：`
  ...
  const messages: ChatMessage[] = [
    ...
    console.log('✅ [DEEP-QUESTIONS] Got response from chatWithDoubao')
    ...
    try {
      ...
      // 🔥 确保只返回一个问题（如果返回多个，只取第一个）
      let questions = parsed.questions || []
      if (Array.isArray(questions) && questions.length > 1) {
        console.warn('⚠️ [DEEP-QUESTIONS] AI返回了多个问题，只取第一个:', questions)
    ...
    console.error('💥 [DEEP-QUESTIONS] Error in generateDeepQuestions:', error)
