...
- 沟通风格：${subconsciousData.communicationStyle?.slice(0, 3).join('; ') || '未分析'}
- 压力反应：${subconsciousData.stressResponse?.slice(0, 3).join('; ') || '未分析'}
- 人际挑战：${subconsciousData.interpersonalChallenges?.slice(0, 3).join('; ') || '未分析'}
- 对话洞察：${subconsciousData.conversationInsights?.slice(0, 3).join('; ') || '未分析'}
...
只返回JSON，不要其他文字！`
      ...
      return {
        ...
        otherCharacters: [],
        isPsychodrama: true,
        innerMonologue: `内心独白：面对"${emotion.trigger}"，产生强烈的${emotion.type}情绪。${emotion.quote}`,
        surfaceVsInner: `表面上保持冷静和专业，内心却充满${emotion.type}的情绪`,
        consciousnessStream: `${emotion.quote}...${emotion.trigger}...`,
        psychologicalSymbolism: `把这个情境看作一场充满讽刺的场景`,
...
【检测到的情绪】
${emotionResult.emotions.map(e => `${e.type}: ${e.intensity}分 - ${e.reason}`).join('\n')}
        ...
        if (psychodramaScene) {
          ...
          const psychodramaSceneData = {
            ...
            visualDetails: {
              ...
              mood: psychodramaScene.surfaceVsInner || psychodramaScene.innerMonologue
            },
            detailedPrompt: `PSYCHODRAMA Scene: ${psychodramaScene.imagePrompt}`,
            imagePrompt: psychodramaScene.imagePrompt,  // 添加单独的imagePrompt字段
            isPsychodrama: true,
            innerMonologue: psychodramaScene.innerMonologue,
            surfaceVsInner: psychodramaScene.surfaceVsInner,
            consciousnessStream: psychodramaScene.consciousnessStream,
            psychologicalSymbolism: psychodramaScene.psychologicalSymbolism,
  ...
  static async integratePsychodramaIntoContent(
    ...
    const psychodramaSceneData = {
      ...
      visualDetails: {
        ...
        mood: `${psychodramaScene.surfaceVsInner || psychodramaScene.innerMonologue || psychodramaScene.emotionalTrigger}`
      },
    ...
    return {
      ...
      hasPsychodrama: true,
      psychodramaSceneIndex: insertAfterIndex + 1,
